...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($matkhau !== $xacnhanmatkhau) {
        ...
        // Kiểm tra xem email và id đã tồn tại chưa
        $checkEmail = "SELECT * FROM nguoidung WHERE email=?";
        $checkId = "SELECT * FROM nguoidung WHERE id=?";
        ...
        if ($resultEmail->num_rows > 0) {
            ...
            $sql = "INSERT INTO nguoidung (id, hoten, email, matkhau, trangthai) VALUES (?, ?, ?, ?, ?)";
            $trangthai = 1; // Trạng thái mặc định

            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssi", $id, $hoten, $email, $matkhau_mahoa, $trangthai);
